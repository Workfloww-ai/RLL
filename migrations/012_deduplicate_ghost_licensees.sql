-- Migration: 012_deduplicate_ghost_licensees.sql
-- Description: Clean up ghost 0-sales duplicate licensees in database and update RPCs to deduplicate by normalized licensee name.

-- 1. Delete ghost 0-sales duplicate licensees where a licensee with sales exists for the same normalized name in the group
WITH ghost_pairs AS (
    SELECT 
        l1.licensee_id AS ghost_id,
        l2.licensee_id AS real_id
    FROM (
        SELECT 
            licensee_id, 
            group_id,
            UPPER(REGEXP_REPLACE(REGEXP_REPLACE(TRIM(licensee_name), '\s+', ' ', 'g'), '\s*-\s*', '-', 'g')) AS norm_name
    ...
    JOIN (
        ...
        FROM public.licensees
        WHERE is_active = true
    ) l2 ON l1.group_id = l2.group_id AND l1.norm_name = l2.norm_name AND l1.licensee_id != l2.licensee_id
    WHERE l1.licensee_id NOT IN (
        SELECT DISTINCT licensee_id FROM public.sales_monthly_summary
        UNION
        SELECT DISTINCT licensee_id FROM public.sales_daily_summary
    )
    AND l2.licensee_id IN (
        SELECT DISTINCT licensee_id FROM public.sales_monthly_summary
        UNION
        SELECT DISTINCT licensee_id FROM public.sales_daily_summary
    )
)
DELETE FROM public.licensees
WHERE licensee_id IN (SELECT ghost_id FROM ghost_pairs);

-- 2. Normalize all licensee names in public.licensees to collapse extra spaces
UPDATE public.licensees
SET licensee_name = UPPER(REGEXP_REPLACE(REGEXP_REPLACE(TRIM(licensee_name), '\s+', ' ', 'g'), '\s*-\s*', '-', 'g'));

-- 3. Update get_group_licensees_summary_json RPC with DISTINCT ON deduplication safeguard
    ...
    SELECT jsonb_agg(row_to_json(lic)::jsonb) FROM (
        ...
        group_lics AS (
            ...
            ORDER BY 
                ...
                (l.licensee_id IN (SELECT licensee_id FROM public.sales_monthly_summary) OR l.licensee_id IN (SELECT licensee_id FROM public.sales_daily_summary)) DESC
        ),
        brand_cnt AS (
            SELECT
                sms.licensee_id,
...
-- 4. Update get_cascading_groups_summary_json RPC with DISTINCT ON licensee deduplication safeguard
