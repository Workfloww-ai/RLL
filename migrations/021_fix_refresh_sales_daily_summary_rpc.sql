-- Migration: 021_fix_refresh_sales_daily_summary_rpc.sql
-- Description: Support optional p_depot_id in refresh_sales_daily_summary_for_date for resilient fallback chunking.
-- Preserves Licensee's Headquarters credit (COALESCE(l.headquarters_id, d.headquarters_id)).

DROP FUNCTION IF EXISTS public.refresh_sales_daily_summary_for_date(DATE);
DROP FUNCTION IF EXISTS public.refresh_sales_daily_summary_for_date(DATE, UUID);

CREATE OR REPLACE FUNCTION public.refresh_sales_daily_summary_for_date(
    p_sale_date DATE,
...
BEGIN
    ...
    ELSE
        DELETE FROM public.sales_daily_summary WHERE sale_date = p_sale_date AND depot_id = p_depot_id;
    END IF;
    ...
    WITH agg_sales AS (
        ...
        WHERE sf.sale_date = p_sale_date
          AND (p_depot_id IS NULL OR sf.depot_id = p_depot_id)
    ...
    depot_roles AS (
        ...
        WHERE UPPER(r.role_name) IN ('TSM', 'ASE')
          AND (p_depot_id IS NULL OR ud.depot_id = p_depot_id)
    ),
    depot_tsm AS (
...
GRANT EXECUTE ON FUNCTION public.refresh_sales_daily_summary_for_date(DATE, UUID) TO authenticated, service_role, anon;
