...
async def login(credentials: LoginRequest, request: Request, response: Response):
    ...
    try:
        ...
        if ur_res.data:
            ...
            preferred_role = next((r for r in active_roles if r.lower() in ["admin", "super_admin", "super admin", "leader", "developer"]), None)
            if preferred_role:
    ...
    # Guardrail: Enforce Web-only roles (Admin, Super Admin, Leader, Developer)
    if role_name.lower() not in ["admin", "super_admin", "super admin", "leader", "developer"]:
