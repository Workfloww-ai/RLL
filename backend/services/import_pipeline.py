...
class ImportPipelineEngine:
    ...
    async def prepare_file_upload(self, file: UploadFile, user_id: str) -> Tuple[Dict[str, Any], str]:
        ...
        batch_record = {
            ...
            "uploaded_by": user_id if len(str(user_id)) == 36 else None,
            "total_rows": 0,
            "imported_rows": 0,
            "duplicate_rows": 0,
            "failed_rows": 0,
            ...
            "is_active": True,
            "remarks": "File upload accepted. Ingestion started in background.",
            "created_at": now_iso,
            "updated_at": now_iso
        }
    ...
    def process_file_background(self, batch_record: Dict[str, Any], temp_path: str, user_id: str) -> Dict[str, Any]:
        global sales_counter
        start_time = time.time()
        batch_id = batch_record.get("upload_batch_id", 1)
        filename = batch_record.get("file_name", "uploaded_file.xlsx")
        filename_lower = filename.lower()
        ...
        try:
            ...
            if filename_lower.endswith(".numbers"):
                try:
                    ...
                    logger.error(f"Error parsing .numbers file: {e}")
            elif filename_lower.endswith(".csv"):
                df_temp = pd.read_csv(temp_path, header=None)
                raw_rows = df_temp.values.tolist()
            else:
                df_temp = pd.read_excel(temp_path, header=None, engine="openpyxl" if filename_lower.endswith(".xlsx") else None)
                raw_rows = df_temp.values.tolist()
            ...
            logger.exception(f"Error processing background batch {batch_id}: {exc}")
            batch_record.update({
                "upload_status": "failed",
                "remarks": f"Processing error: {str(exc)}",
                "processing_time_seconds": round(time.time() - start_time, 2)
            })
            upload_batches_db[batch_id] = batch_record
            self._update_batch_record_in_supabase(batch_id, batch_record)
